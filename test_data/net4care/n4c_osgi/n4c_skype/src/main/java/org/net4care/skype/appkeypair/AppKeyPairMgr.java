...
package org.net4care.skype.appkeypair; 
...
import com.skype.util.PemReader; 
		...
		public boolean resolveAppKeyPairPath(String pathName) { 
			...
			if (i >= j) { 
				if (pathName.charAt((pathName.length() - 3)) == '.') { 
					...
					Net4CareSession.myConsole.printf("%s/resolveAppKeyPairPath: Unrecognized Certificate file suffix.%n\tPathname: %s%n\tRecognized suffixes:%n",  
							MY_CLASS_TAG, pathName); 
					j = KEY_PAIR_FILE_SUFFIXES.length; 
					for (i = 0; i < j; i++) { 
						Net4CareSession.myConsole.printf("\t\t%s%n", KEY_PAIR_FILE_SUFFIXES[i]); 
					} 
					return (false); 
				} 
				else { 
		    ...
		    try { 
		    	...
		    	Net4CareSession.myConsole.printf("%s/resolveAppKeyPairPath: Specified pathname is NULL%n",  
		    			MY_CLASS_TAG); 
	    	...
	    	if (!tmpFile.exists()) { 
	    		Net4CareSession.myConsole.printf("%s/resolveAppKeyPairPath: Certificate file doesn't exist:%n\t%s%n", 
	    				MY_CLASS_TAG, pemFilePathname); 
               return (true); 
		    } 
 
	    	Net4CareSession.myConsole.printf("%s/resolveAppKeyPairPath: Found certificate file:%n\t%s%n", 
         		   MY_CLASS_TAG, pemFilePathname); 
		    return (true); 
		} 
	 ...
	 *  
	 * @see #pemFilePathname 
	 * @see org.net4care.skype.api.Net4CareSession.util.MySession#myClientConfiguration 
	 * @since 2.0 
	 */ 
	public boolean isValidCertificate() { 
	    ...
	    try { 
	        ...
	        privateKeyData = myPemReader.getKey(); 
               ...
               Net4CareSession.myConsole.printf("%s/isValidCertificate: Could not find certificate file:%n\t%s%n",  
            		   MY_CLASS_TAG, pathName); 
               return (false); 
           } 
	    catch (IOException e2) { 
	    	Net4CareSession.myConsole.printf("%s/isValidCertificate: Unable to read certificate file:%n\t%s%n", 
            		   MY_CLASS_TAG, pathName); 
               return (false); 
	    } 
	    catch (InvalidKeySpecException e3) { 
	    	Net4CareSession.myConsole.printf("%s/isValidCertificate: Certificate file %s contains invalid certficate data.%n", 
		    		MY_CLASS_TAG, pathName); 
		    return(false); 
	    } 
	     
	    if ((x509CertificateData != null) && (privateKeyData != null)) { 
	    	Net4CareSession.myConsole.printf("%s/isValidCertificate: Certificate has valid format%n", 
	         	   MY_CLASS_TAG); 
	    	return (true); 
	    } 
	    else { 
	    	Net4CareSession.myConsole.printf("%s/isValidCertificate: Certificate has invalid format%n", 
		            MY_CLASS_TAG); 
	    	return (false); 
	    } 
	} 
